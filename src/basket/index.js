...
const checkoutBasket = async (event) => {
  ...
  // expected request payload -> {userName, attributes[firstName, lastName, email,...]}
  ...
  // 2-> Create an event json object with basket items, calculate total price,
  // prepare order, create json data to send to ordering microservice
  let checkoutPayload = prepareOrderPayload(checkoutRequest, basket);

  // 3-> publish an event to eventBridge - this will be subscribed by ordering microservice.
